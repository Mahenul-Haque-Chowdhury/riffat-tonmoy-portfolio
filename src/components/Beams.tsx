...
type UniformLike = THREE.IUniform & Record<string, unknown>;
...
function extendMaterial(
  ...
  if (typeof defaults.roughness === "number") uniforms.roughness.value = defaults.roughness;
  if (typeof defaults.metalness === "number") uniforms.metalness.value = defaults.metalness;
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    ...
    if (v !== null && typeof v === "object" && "value" in (v as Record<string, unknown>)) {
      uniforms[key] = v as UniformLike;
    } else {
      uniforms[key] = { value: v } as UniformLike;
    }
  });
...
>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    ...
    const uniforms = mesh.current.material.uniforms as Record<string, UniformLike>;
